...
contract CurciferDex is ReentrancyGuard, Ownable {
    ...
    address private constant USDC_TOKEN_CONTRACT =
    ...
    function addTokenAddress(address _erc20FeeTokenAddress, uint256 _fee)
        public
        onlyOwner
    {
    ...
    function createPersonalProvider(
        ...
        require(
            _providerToken.balanceOf(msg.sender) >= _providerTokenValue,
            "Provider Token Balance not enough"
        );
        require(
            _selectedErc20IndexForPayFee < tokenContracts.length,
            "Provider select invalid fee index"
        );

        uint256 _fee = fees[_selectedErc20IndexForPayFee];
        address _feeTokenAddress = tokenContracts[_selectedErc20IndexForPayFee];
        require(
            _providerToken.allowance(msg.sender, address(this)) >=
                _providerTokenValue,
            "Provider token not enough allowance"
        );
        require(
            IERC20(_feeTokenAddress).allowance(msg.sender, address(this)) >=
                _fee,
            "Provider fee token not enough allowance"
        );
        require(
            IERC20(_feeTokenAddress).balanceOf(msg.sender) >= _fee,
            "Provider fee token Balance not enough"
        );
        ...
        IERC20(_providerToken).safeTransferFrom(
            msg.sender,
            address(this),
            _providerTokenValue
        );
        ...
        activePersonalProviders[msg.sender].push(
            ProviderData(
    ...
    function fulfillPersonalTradeOrder(
        address _providerAddress,
        uint256 _exchangeIndex
    ) public {
        require(
            msg.sender != _providerAddress,
            "Provider can not fulfill own order"
        );

        ProviderData[] memory providerExchange = activePersonalProviders[
            _providerAddress
        ];
        require(
            _exchangeIndex < providerExchange.length,
            "This execution is trying to execute index nothing"
        );

        ProviderData memory exchangeData = providerExchange[_exchangeIndex];
        require(
            IERC20(exchangeData.targetToken).allowance(
                msg.sender,
                address(this)
            ) >= exchangeData.targetTokenValue,
            "Target token allowance target not enough"
        );
        require(
            IERC20(exchangeData.targetToken).balanceOf(msg.sender) >=
                exchangeData.targetTokenValue,
            "Target Token Balance not enough"
        );
        require(
            exchangeData.orderTaker == address(0x0),
            "This order was already fulfilled"
        );

        exchangeData.orderTaker = msg.sender;
        activePersonalProviders[_providerAddress][
            _exchangeIndex
        ] = exchangeData;
        IERC20(exchangeData.targetToken).safeTransferFrom(
            msg.sender,
            address(this),
            exchangeData.targetTokenValue
        );
    ...
    function executePersonalTrade(
        address _providerAddress,
        uint256 _exchangeIndex
    ) public nonReentrant {
        require(
            msg.sender != _providerAddress,
            "Provider can not execute own order"
        );
        ...
        require(
            _exchangeIndex < providerExchange.length,
            "This execution is trying to execute invalid index"
        );

        ProviderData memory exchangeData = providerExchange[_exchangeIndex];
        require(
            exchangeData.orderTaker == msg.sender,
            "This order is not for this address"
        );
        require(
            exchangeData.orderTaker != address(0x0),
            "The order is not fulfill yet"
        );

        uint256 _activePersonalProviderLength = activePersonalProviders[
            _providerAddress
        ].length;
        ...
        if (_activePersonalProviderLength > 0) {
            ...
            activePersonalProviders[_providerAddress][
                ...
                _activePersonalProviderLength - 1
            ];
        }
        // remove last index
        activePersonalProviders[_providerAddress].pop();

        IERC20(exchangeData.targetToken).safeTransfer(
            _providerAddress,
            exchangeData.targetTokenValue
        );
        IERC20(exchangeData.providerToken).safeTransfer(
            msg.sender,
            exchangeData.providerTokenValue
        );
    }

    function cancelOrderTrade(address _providerAddress, uint256 _exchangeIndex)
        public
        nonReentrant
    {
        require(
    ...
    function cancelProviderTrade(uint256 _exchangeIndex) public nonReentrant {
        ...
        if (_activePersonalProviderLength > 0) {
            // swap last index to selected index
            activePersonalProviders[msg.sender][
                _exchangeIndex
            ] = activePersonalProviders[msg.sender][
                _activePersonalProviderLength - 1
            ];
    ...
    function getPersonalProvider(uint256 _page, uint256 _limitSize)
        external
        view
        returns (address[] memory)
    {
        ...
        uint256 cursor = (_page - 1) * _limitSize;
        address[] memory _personalProviders = new address[](_limitSize);
        for (uint256 i = cursor; i < (_page * _limitSize - 1); i++) {
